...
from .models import (CustomUser, Activity,
 CashWithdraw, HomePage, Coin,
  Exchange, Message, Video, CashRate, DailyCombo, DailyComboClaim,
  OnlineShop, CustomerPurchase, PurchaseCoin, WireCoin)
=======
from .models import (CustomUser, Profile,Publish, Photo, 
Video, PaymentInfo, Invest, Settings,
LikePublish, RatePublish, CommentPublish,
...
class CustomUserSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            ...
            'phone', 'image_url', 'code', 'two_factor_authentication'
        ]
...
class PurchaseCoinSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PurchaseCoin
        fields = [
            'id', 'user', 'username', 'no_of_coins', 'is_paid', 'timestamp'
        ]

    def get_username(self, obj):
        # Assuming 'user' is a ForeignKey to the CustomerUser model
        try:
            # Retrieve the related CustomerUser and return the username
            return obj.user.username
        except CustomerUser.DoesNotExist:
            return None
            
class WireCoinSerializer(serializers.ModelSerializer):
    username = serializers.SerializerMethodField(read_only=True)
    
    class Meta:
        model = WireCoin
        fields = [
            'id', 'user', 'username', 'recepient', 'no_of_coins',
            'is_paid', 'timestamp'   
        ]
...
class CashRateSerializer(serializers.ModelSerializer):
    class Meta:
        model = CashRate
        fields = [
            'USD', 'EUR', 'JPY', 
            ...
            'CHF', 'CNY','HKD',
            'NZD', 'INR', 'BRL', 'RUB', 'ZAR',
            'AED', 'SAR', 'MXN', 'SHD', 'KRW', 'ETB',
            'KES', 'TZS', 'GHS', 'UGX', 'XAF', 'XOF',
...
class OnlineShopSerializer(serializers.ModelSerializer):
    class Meta:
        model = OnlineShop 
        fields = [
        'id', 'item_name', 'item_image', 'stock_balance', 'price'
        ]


class DailyComboSerializer(serializers.ModelSerializer):
    class Meta:
        model = DailyCombo
        fields = [
            'id', 'activity_one', 'activity_two',
            'activity_three', 'activity_four', 'activity_five',
            'deadline', 'accomplishment_value',
            
        ]
class DailyComboClaimSerialzier(serializers.ModelSerializer):
    class Meta:
        model = DailyComboClaim
        fields = [
            'user', 'combo', 'is_claimed', 'timestamp'
        ]
        
class  CustomerPurchaseSerializer(serializers.ModelSerializer):
    shop = OnlineShopSerializer(read_only=True)
    shop_id = serializers.PrimaryKeyRelatedField(
        queryset=OnlineShop.objects.all(), source='shop', write_only=True
    )
    username = serializers.SerializerMethodField(read_only=True)
    ...
    class Meta:
        ...
        fields = [
           'id', 'username',  'user', 'shop', 'shop_id', 'is_paid', 'is_deliverd'
        ]
...
class RegisterStaffSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        'username', 'password', 'passport',  'password2', 'email')
...
class ActivitySerializer(serializers.ModelSerializer):
    class Meta:
        model = Activity
        fields= ['id', 'activity_type','coins_earned', 'timestamp']
...
>>>>>>> 2b11a5b (first commit)
    class Meta:
        model = Message
        fields = ['id', 'content', 'user', 'receiver', 'timestamp' ]

class HomePageSerializer(serializers.ModelSerializer):
    class Meta:
        model = HomePage
        fields= ['id', 'user','daily_reward', 'daily_cipher', 'daily_combo']


class CashWithdrawSerializer(serializers.ModelSerializer):
    class Meta:
        model = CashWithdraw
        fields= ['id', 'paypal_email_address','currency',
        'recepient_name', 'amount_to_send',
         'timestamp', 'is_processed', 'user']

class CoinSerializer(serializers.ModelSerializer):
    class Meta:
        model = Coin
        fields= ['id', 'code','value', 'expiry_date', 'timestamp']


class ExchangeSerializer(serializers.ModelSerializer):
    class Meta:
        model = Exchange
        fields= ['id', 'currencytype','value', 'expiry_date', 'timestamp']
...
class PaymentInfoSerializer(serializers.ModelSerializer):
    class Meta:
        model = PaymentInfo
        fields = ['_id', 'bank_info', 'bank_account' ]
...
class CommentPublishSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CommentPublish
        fields = ['_id', 'parent', 'user', 'comment', 'created_at', 'user_details', 'photos', 'replies']

    def get_user_details(self, obj):
        user = CustomUser.objects.filter(id=obj.user.id).first()
        if user:
            return CustomUserSerializer(user).data
        return None

    def get_replies(self, obj):
        replies = CommentPublish.objects.filter(parent_comment=obj)
        if replies.exists():
            return CommentPublishSerializer(replies, many=True).data
        return []
    
    def get_created_at(self, obj):
        # Format created_at as 'Jan 16, 14:12'
...
class CommentInvestSerializer(serializers.ModelSerializer):
    user_details = serializers.SerializerMethodField()
    replies = serializers.SerializerMethodField()

    class Meta:
        model = CommentInvest
        fields = ['_id', 'parent', 'user', 'comment', 'photos', 
        'user_details', 'replies', 'created_at']
    ...
    def get_replies(self, obj):
        replies = CommentInvest.objects.filter(parent_comment=obj)
        if replies.exists():
            return CommentInvestSerializer(replies, many=True).data
        return []
...
class PublishSerializer(serializers.ModelSerializer):
    ...
    payment_detail = PaymentInfoSerializer(many=True, read_only=True)
    ...
    payments = serializers.ListField(child=serializers.CharField(), write_only=True)
    class Meta:
        model = Publish
        fields = [
            ...
            'no_of_photos', 'no_of_videos', 'photos', 'videos', 'payments', 'photos_detail', 'videos_detail', 'payment_detail',
    ...
    def create(self, validated_data):
        ...
        for bank_info in payment_data:
            PaymentInfo.objects.create(parent=publish, bank_info=bank_info)
...
class PublishRetrieveSerializer(serializers.ModelSerializer):
    ...
    payment_detail = PaymentInfoSerializer(many=True, read_only=True, source='payments')  
    like_count = serializers.SerializerMethodField()
    ...
    videos = serializers.ListField(child=serializers.CharField(), write_only=True)
    payments = serializers.ListField(child=serializers.CharField(), write_only=True)
    comment = CommentPublishSerializer(many=True, read_only=True, source='commentspublish')

    class Meta:
        model = Publish
        fields = [
            '_id', 'parent', 'image_url', 'author', 'email', 'phone', 'organization', 'submission_type',
            'title', 'short_description', 'key_features_and_goals', 'target_audience', 
            'development_stage', 'amount_needed', 'how_will_funds_will_be_used', 
            'market_overview', 'competitors', 'potential_user_impact', 'uniqueness', 
            'no_of_photos', 'no_of_videos', 'photos', 'videos','payments',
             'photos_detail', 'videos_detail',  'payment_detail',
            'created_at', 'like_count', 'average_rating', 'total_rating', 'comment', 'comment_count',
        ]
    ...
    # Use the correct related_name 'comments' to count related CommentPublish instances
        return obj.commentspublish.count()
    ...
    def get_image_url(self, obj):
        request = self.context.get('request')
        if obj.parent and obj.parent.image_url:
            # Construct the full URL using the request object and MEDIA_URL
            relative_url = obj.parent.image_url.url  # Ensure this uses the `url` property of the field
            if request:
                return urljoin(request.build_absolute_uri('/'), relative_url)
            return urljoin(settings.MEDIA_URL, relative_url)  # Fallback
        return None
...
class InvestRetrieveSerializer(serializers.ModelSerializer):
    ...
    comment_count = serializers.SerializerMethodField()
    created_at = serializers.SerializerMethodField() 
    comment = CommentInvestSerializer(many=True, read_only=True, source='commentinvest')

    class Meta:
        model = Invest
        fields = [
            '_id', 'parent', 'title', 'budget', 'description', 
            'category', 'investment_type',
            'created_at', 'like_count', 'average_rating',
             'total_rating', 'comment', 'comment_count',
        ]
    ...
    def get_comment_count(self, obj):
    # Use the correct related_name 'commentinvest' to count related CommentInvest instances
        return obj.commentinvest.count()
...
class ProfilesSerializer(serializers.ModelSerializer):
    ...
    def get_user_id(self, obj):
        # Access the parent field directly and handle reverse relationship
        if obj.parent:
...
class CustomUserProfileSerializer(serializers.ModelSerializer):
    class Meta:
        model = CustomUser
        fields = ['id', 'first_name', 'last_name', 'email', 'phone', 'image_url']

    def update(self, instance, validated_data):
        # Update fields in the main serializer
...
class UserProfileActivitySerializer(serializers.Serializer):
    publishes = PublishRetrieveSerializer(many=True)
    investments = InvestRetrieveSerializer(many=True)   
